...
php.require_once = function (filename) {
    ...
    // BEGIN STATIC
    try { // We can't try to access on window, since it might not exist in some environments, and if we use "window"
...
php.require = function (filename) {
    ...
    var js_code = php.file_get_contents(filename);
